...
std::string inline GetPlayerName(int playerid) {
...
public:
	...
	bool ChangeWeather(int weather) const
		{ return ::SetPlayerWeather(id_, weather); }
	bool ForceReselectPlayerClass() const
	...
	bool SetAnimation(const char *animlib, const char *animname, float fDelta, bool loop, bool lockx, bool locky, bool freeze, int time, bool forcesync = false) const
		{ return ::ApplyAnimation(id_, animlib, animname, fDelta, loop, lockx, locky, freeze, time, forcesync); }
	bool UnsetAnimations(bool forcesync = false) const
	...
	bool CameraAttachToObject(int objectid) const
		{ return ::AttachCameraToObject(id_, objectid); }
	bool CameraAttachToPlayerObject(int playerobjectid) const
		{ return ::AttachCameraToPlayerObject(id_, playerobjectid); }
	bool CameraInterpolatePos(float FromX, float FromY, float FromZ, 
	...
	bool CameraInterpolateLookAt(float FromX, float FromY, float FromZ, 
			float ToX, float ToY, float ToZ, int time, int cut = CAMERA_CUT) const
	...
	bool BanNow() const
		{ return ::Ban(id_); }
	bool BanNowEx(const char *reason) const
		{ return ::BanEx(id_, reason); }
	bool BanNowEx(const std::string &reason) const
		{ return ::BanEx(id_, reason.c_str()); }
	bool GetNetworkStatistics(char *retstr, int size) const
		{ return ::GetPlayerNetworkStats(id_, retstr, size); }
	int GetMenu() const
		{ return ::GetPlayerMenu(id_); }
	bool SendChatMessage(int color, const char *message) const
		{ return ::SendClientMessage(id_, color, message); }
	bool SendChatMessage(int color, const std::string &message) const
	...
	bool EnableSelectTextDraw(int hovercolor) const
		{ return ::SelectTextDraw(id_, hovercolor); }
	bool DisableSelectTextDraw() const
