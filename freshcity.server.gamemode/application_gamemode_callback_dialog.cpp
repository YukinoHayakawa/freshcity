...
DIALOG(DIALOG_RACING_MAIN, DIALOG_STYLE_LIST, "竞赛", "确定", "取消", false) {
	switch(listitem) {
	case 0: /* Start */
		DlgMgr.Show(DIALOG_RACING_START_NAME, "赛道名称", player.GetId());
		break;

	case 1: /* Create */
		DlgMgr.Show(DIALOG_RACING_CREATE_NAME, "赛道名称", player.GetId());
		break;
...
DIALOG(DIALOG_RACING_START_NAME, DIALOG_STYLE_INPUT, "发起竞赛", "确定", "取消", false) {
	boost::shared_ptr<Racing> session(new Racing(inputtext, player.GetId()));
	session->Join(player);
	RacingMgr.Add(player.GetId(), session);
}

DIALOG(DIALOG_RACING_CREATE_NAME, DIALOG_STYLE_INPUT, "创建赛道", "确定", "取消", false) {
	if(player.HasVar("race_joined")) throw std::runtime_error("已经参加了别的竞赛");
